...
// หมายเหตุ: Utility functions (showValidationError, isValidEmail ฯลฯ)
//           อยู่ใน utils.js — โหลดก่อนไฟล์นี้เสมอ
...
function confirmSaveUserData() {
    ...
    // 1. Username: อย่างน้อย 1 ตัว, สูงสุด 50 ตัว
    if (username.length < 1) {
        showValidationError(usernameEl, 'กรุณากรอก Username อย่างน้อย 1 ตัวอักษร');
        return;
    }

    // 2. Password: 5–30 ตัว
    if (password.length < 5) {
        showValidationError(passwordEl, 'Password ต้องมีอย่างน้อย 5 ตัวอักษร');
        return;
    }
    if (password.length > 100) {
        showValidationError(passwordEl, 'Password ยาวได้สูงสุด 100 ตัวอักษร');
        return;
    }

    // 3. Phone: ถ้ากรอกมา ต้องมี 10 หลักพอดี
    if (phoneRaw.length > 0 && phoneRaw.length !== 10) {
        showValidationError(phoneEl, `เบอร์โทรศัพท์ต้องมี 10 หลักพอดี (ปัจจุบัน ${phoneRaw.length} หลัก)`);
        return;
    }

    // 4. Email: ถ้ากรอกมา ต้องถูกรูปแบบ
    if (email.length > 0 && !isValidEmail(email)) {
        showValidationError(emailEl, 'รูปแบบ Email ไม่ถูกต้อง (เช่น name@example.com)');
        return;
    }

    // ผ่านทุกเงื่อนไข — เปิด Confirm Modal
